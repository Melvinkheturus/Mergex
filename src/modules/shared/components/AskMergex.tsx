...
export function AskMergex({
    ...
    const handleMinimalSubmit = (textOverride?: string) => {
        const text = textOverride || value;
        if (!text.trim()) return;
        router.push(`/ask-mergex?q=${encodeURIComponent(text.trim())}`);
    };
    ...
    return (
        <div className={cn(
            ...
            <div className="relative z-10 w-full flex flex-col h-full max-w-3xl pt-12 pb-6 px-4 md:px-8">
                ...
                {!hasStarted && (
                    ...
                    >
                        <div className="space-y-4 text-center mb-10">
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }} className="inline-block">
                ...
                <motion.div
                    layout
                    className={cn('w-full', !hasStarted ? 'mx-auto' : 'mt-auto')}
